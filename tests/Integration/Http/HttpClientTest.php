...
use Invertus\Http\Client\Events\RequestSending;
use Invertus\Http\Client\Events\ResponseReceived;
use Invertus\Http\Client\Factory;
use Invertus\Http\Client\PendingRequest;
use Invertus\Http\Client\Pool;
use Invertus\Http\Client\Request;
use Invertus\Http\Client\RequestException;
use Invertus\Http\Client\Response;
use Invertus\Http\Client\ResponseSequence;
...
{
    /**
     * @var \Invertus\Http\Client\Factory
     */
    protected $factory;
    ...
    public function testUrlsCanBeStubbedByPath()
    {
        ...
        $this->assertSame('foo', $fooResponse->json()['page']);
        $this->assertSame('bar', $barResponse->json()['page']);
        $this->assertSame('fallback', $fallbackResponse->json()['page']);
    ...
    public function testCanSendMultipartDataWithBothSimplifiedAndExtendedParameters()
    {
        ...
        $this->factory->assertSent(function (Request $request) {
            return $request->url() === 'http://foo.com/multipart' &&
                ...
                $request->data()[0]['name'] === 'foo' &&
                $request->data()[0]['contents'] === 'bar' &&
                $request->data()[1]['name'] === 'foobar' &&
                $request->data()[1]['contents'] === 'data' &&
                $request->data()[1]['headers']['X-Test-Header'] === 'foo';
        });
    }
    ...
    public function testGetWithQueryWontEncode()
    {
        ...
        $this->factory->assertSent(function (Request $request) {
            return $request->url() === 'http://foo.com/get?foo;bar;1;5;10&page=1'
                && ! isset($request->data()['foo'])
                && ! isset($request->data()['bar'])
                && $request->data()['page'] === '1';
        });
    }
    ...
    public function testGetWithArrayQueryParamEncodes()
    {
        ...
        $this->factory->assertSent(function (Request $request) {
            return $request->url() === 'http://foo.com/get?foo%3Bbar%3B%20space%20test=laravel'
                && $request->data()['foo;bar; space test'] === 'laravel';
        });
    }
    ...
    {
        $this->markTestSkipped('TODO mock');
    ...
    public function testRequestExceptionIsThrownIfStatusCodeIsSatisfiedWithClosure()
    {
        ...
        try {
            $this->factory->get('http://foo.com/api')->throwIfStatus(function ($status) {
                return $status === 400;
            });
        } catch (RequestException $e) {
            $exception = $e;
    ...
    public function testRequestExceptionIsThrownUnlessStatusCodeIsSatisfied()
    {
        ...
        try {
            $this->factory->get('http://foo.com/api/400')->throwUnlessStatus(function ($status) {
                return $status === 500;
            });
        } catch (RequestException $e) {
            $exception = $e;
